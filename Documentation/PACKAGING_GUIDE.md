...
3. Import the exported ZIP (`build/IconsOfIOAwards_v1.0.0.zip`) via Admin Center → Solutions → Import.
...
- PAC not found: Install via Homebrew or dotnet global tool; the scripts will use `~/.dotnet/tools/pac` or `%USERPROFILE%\.dotnet\tools\pac.exe` automatically when available.
